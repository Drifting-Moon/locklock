...
export default function TrafficDashboard() {
  ...
  useEffect(() => {
    const saved = localStorage.getItem("gridlock_active_tab");
    if (saved) setActiveTab(saved);
  }, []);

  useEffect(() => {
    localStorage.setItem("gridlock_active_tab", activeTab);
  }, [activeTab]);
        ...
        <html>
          ...
          <body>
            <div style="display: flex; align-items: center; margin-bottom: 20px;">
              <div style="width: 32px; height: 32px; background: #3e52ff; border-radius: 8px; display: flex; align-items: center; justify-content: center; color: white; font-weight: bold; font-family: monospace; font-size: 18px; margin-right: 12px;">G</div>
              <h1 style="margin: 0; color: #111; font-weight: 800;">Urban Intel Dispatch Plan</h1>
            </div>
            <div class="meta">
            ...
            <div class="footer">
              <strong>Generated by AI Patrol Engine.</strong> Report generated dynamically from the Urban Intel analytics platform using live Overpass OSM datasets and BPR traffic congestion physics.
            </div>
            ...
            <script>
              ...
              if (data.hotspots.length === 0) {
                ...
                const sorted = [...data.hotspots].sort((a,b) => b.bprDelay - a.bprDelay).slice(0, 5);
                sorted.forEach((h, index) => {
                  ...
                  tr.innerHTML = \`
                    <td><strong>#\${index + 1} \${h.locationName}</strong></td>
      ...
      <div className="flex h-full w-full relative z-10">
        ...
        <div className="flex-1 flex flex-col h-screen overflow-hidden relative">
          ...
          >
            ...
            {activeTab === "Economics" && <EconomicCalculator selectedHotspot={selectedHotspot} />}
            {activeTab === "Enforcement" && <EnforcementTab />}
            {activeTab === "Detection" && <DetectionTab selectedHotspot={selectedHotspot} />}
            {activeTab === "CCTV Intel" && <CctvIntelTab selectedHotspot={selectedHotspot} />}
          </main>
        </div>
      </div>
